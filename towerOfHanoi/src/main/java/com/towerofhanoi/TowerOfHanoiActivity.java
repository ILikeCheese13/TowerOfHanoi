...
public class TowerOfHanoiActivity extends SimpleBaseGameActivity {
	...
	protected Scene onCreateScene() {
        ...
        main_font = FontFactory.create(this.getFontManager(), this.getTextureManager(), 256, 256, BitmapTextureFormat.RGBA_8888, TextureOptions.BILINEAR_PREMULTIPLYALPHA, Typeface.DEFAULT, 20, true, Color.BLACK_ABGR_PACKED_INT);
        main_font.load();
        turnCountText = new Text(0, 0, main_font, "Turn Count: " + Integer.toString(turnCount), this.getVertexBufferObjectManager());
        turnCountText.setPosition(0, 0);
        scene.attachChild(turnCountText);
    ...
    private void isGameOver() {
        if(mStack3.size() == 3) {
            Font main_font = FontFactory.create(this.getFontManager(), this.getTextureManager(), 256, 256, BitmapTextureFormat.RGBA_8888, TextureOptions.BILINEAR_PREMULTIPLYALPHA, Typeface.DEFAULT, 60, true, Color.RED_ABGR_PACKED_INT);
            main_font.load();

            Text gameOverText = new Text(0, 0, main_font, "       Game Over\nFinal Turn Count: " + Integer.toString(turnCount), this.getVertexBufferObjectManager());
            ...
            scene.detachChild(turnCountText);
